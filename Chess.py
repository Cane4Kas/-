...
def filter_safe_moves(board, piece, moves):
	...
	for row, col in moves:
		...
		# --- ДОБАВЛЕНИЕ РОКИРОВКИ ТОЛЬКО ДЛЯ КОРОЛЯ ---
		if isinstance(piece, King):
			castling = piece.get_castling_moves(board)
			for m in castling:
				safe_moves.append(m)
...
def main():
	...
	selected = None
	valid_moves = []
	turn = 'white'  # Белые ходят первыми
	...
	while running:
		...
		for event in pygame.event.get():
			...
			if event.type == pygame.MOUSEBUTTONDOWN:
				...
				if selected is None:  # выбор фигуры
					piece = board[row][col]
					if piece is not None and piece.color == turn:
						selected = piece
						valid_moves = filter_safe_moves(board, piece, piece.get_valid_moves(board))
						print(f"Выбрана фигура: {piece.symbol} на {row},{col}")
				else:  # ход выбранной фигурой # ход выбранной фигурой
					if (row, col) in valid_moves:
						last_move = ((selected.row, selected.col), (row, col))
						# --- РОКИРОВКА ---
						...
						# отмечаем, что король/ладья ходили
...
def main_menu():
	...
	# Уровень сложности ИИ (по умолчанию средний)
	ai_elo = 600
	...
	while running:
		...
		# Отрисовка кнопок с подсветкой при наведении
		mx, my = pygame.mouse.get_pos()
		for key, btn in buttons.items():
			if btn["rect"].collidepoint(mx, my):
				color = (70, 150, 220)  # подсветка
			else:
				color = (80, 80, 80)
		...
		# Отображение уровня сложности ИИ
		if selected_mode == "player_vs_ai":
			difficulty_text = small_font.render(f"Уровень ИИ: Средний", True, (200, 200, 200))
			...
			if event.type == pygame.MOUSEBUTTONDOWN:
				...
				for key, btn in buttons.items():
					if btn["rect"].collidepoint(mx, my):
						...
						selected_mode = key
						if key == "player_vs_ai":
							running = False  # Сразу запускаем игру с ИИ со средним уровнем сложности
						else:
							running = False  # Сразу запускаем игру для не-AI режимов
...
def get_ai_move(board, color, elo):
	"""
	ИИ выбирает ход.
	elo: 100-1200, чем выше — тем умнее ходит.
	"""
	moves = []
	for row in range(ROWS):
		for col in range(COLS):
	...
	# Определяем глубину/случайность хода по Эло
	# Чем выше Эло, тем меньше случайных ходов
	randomness = max(0, 1200 - elo) / 1200  # 0 = максимум, 1 = минимум
	if random.random() < randomness:
		# случайный ход
		return random.choice(moves)

	# ИИ "оценивает" каждый ход по простой стоимости фигур
...
if mode == "player_vs_ai":
	ai_color = "black"  # например, ИИ всегда играет черными
if ai_color == None:
	pygame.time.delay(int(300 * 2.5))  # пауза для имитации "размышления"
	...
	if piece and move:
		...
		animate_move(piece, start, end, board)
		...
		if isinstance(piece, Pawn):
			is_white_end = piece.color == "white" and move[0] == 0
			is_black_end = piece.color == "black" and move[0] == 7
			if is_white_end or is_black_end:
				board[move[0]][move[1]] = Queen(piece.color, move[0], move[1])
...
valid_moves = []
turn = 'white'  # белые ходят первыми
piece_at_cell = None  # Initialize the variable
if piece_at_cell:  # если на клетке есть фигура
	selected = piece_at_cell
...
while running:
	...
	# --- Ход ИИ ---
	if ai_color == turn:
		pygame.time.delay(int(300 * 2.5))  # 0.75 секунды
		piece, move = get_ai_move(board, ai_color, ai_elo)
		if piece and move:
			last_move = ((piece.row, piece.col), move)  # сохраняем последний ход
			# --- Рокировка ---
			if isinstance(piece, King) and abs(move[1] - piece.col) == 2:
				if move[1] == 6:
					rook = board[piece.row][7]
	...
	# Подсветка выбранной фигуры и ходов
	if selected:
		s = pygame.Surface((SQUARE_SIZE, SQUARE_SIZE))
		s.set_alpha(100)
		s.fill((255, 255, 0))
